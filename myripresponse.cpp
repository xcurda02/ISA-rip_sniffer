...
args_s *get_args(int argc, char **argv) {
    ...
    args->ip = nullptr;
    args->metric = 1;
    args->next_hop = nullptr;
    args->route_tag = 0;

    while ((c = getopt(argc, argv, "i:r:n:m:t:h")) != -1) {
        switch (c) {
            case 'i':                                               // Interface
                if (optarg) {
            ...
            case 'r':                                               // Address
                ...
                break;
            case 'n':                                            // Next-hop address

                if (optarg) {
                    string value = optarg;

                    args->next_hop = (struct in6_addr *) malloc(sizeof(struct in6_addr));
                    if (args->next_hop == nullptr) {
                        cerr << "ERROR: malloc() failed" << endl;
                        exit(EXIT_FAILURE);
                    }

                    int s = inet_pton(AF_INET6, value.c_str(), args->next_hop);
                    if (s <= 0) {
                        if (s == 0)
                            cerr << "ERROR: Next-hop address not in presentation format" << endl;
                        else
                            perror("inet_pton");
                        return nullptr;
                    }
...
void print_args(args_s *args) {
    ...
    if (args->next_hop == nullptr)
        strcpy(next_hop, "nullptr");
    else
        inet_ntop(AF_INET6, args->next_hop, next_hop, INET6_ADDRSTRLEN);
...
/**
 * Set next-hop entry members based on args
 * @param args
 * @return
 */
ripng_entry fill_in_nexthop_entry(args_s *args) {
    ripng_entry entry;
    entry.ip_prefix = *args->next_hop;

    memset(&entry.route_tag, 0, sizeof(uint16_t));
    memset(&entry.prefix_length, 0, sizeof(uint8_t));
    memset(&entry.metric, 0xFF, sizeof(uint8_t));
...
int main(int argc, char **argv) {
    ...
    size_t PACKET_SIZE;                                 // Size of RIP part of packet
    ripng_entry entry = fill_in_entry(args);            // Filling in entry
    ripng_entry nexthop_entry;

    if (args->next_hop) {
        PACKET_SIZE = RIP_HEADER_LENGTH + (2 * RIP_ENTRY_LENGTH);
        nexthop_entry = fill_in_nexthop_entry(args);    // Filling in next-hop entry
    } else {
        PACKET_SIZE = RIP_HEADER_LENGTH + RIP_ENTRY_LENGTH;
    }

    u_char *packet = (u_char *) malloc(PACKET_SIZE);    // Pointer to a block of memory that represents a packet
    rip_header header = {RIP_RESPONSE, 1, 0};           // Setting RIP header
    memcpy(packet, &header, RIP_HEADER_LENGTH);         // Inserting header into packet

    /* Inserting entries into packet */
    memcpy(packet + RIP_HEADER_LENGTH, &entry, RIP_ENTRY_LENGTH);
    if (args->next_hop)
        memcpy(packet + RIP_HEADER_LENGTH + RIP_ENTRY_LENGTH, &nexthop_entry, RIP_ENTRY_LENGTH);
    ...
    if(sendto(sock_fd, packet, PACKET_SIZE, 0, (struct sockaddr *) &destination_address, sizeof(sockaddr_in6)) < 0){
        ...
        cout << "Response packet sent" << endl;
    }
